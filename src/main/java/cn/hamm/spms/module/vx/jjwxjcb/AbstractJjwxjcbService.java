...
import cn.hamm.spms.module.vx.jjwxjcb.detail.JjwxjcxxbRepository;
 ...
 */
@Slf4j
public abstract class AbstractJjwxjcbService<
		T extends AbstractJjwxjcbEntity<T, D>,
		R extends BaseRepository<T>,
		D extends JjwxjcxxbEntity,
		DS extends JjwxjcxxbService,
		DR extends JjwxjcxxbRepository
	...
	@Autowired(required = false)
	private JjwxjcxxbRepository detailRepository;
	...
	protected final T afterAppGet(@NotNull T entity) {
		List<JjwxjcxxbEntity> details = detailService.getDetailsById(entity.getId());
		// 将具体类型转换为泛型类型
	...
	private void saveDetails(long entityId, List<D> details) {
		...
		// 获取现有的明细列表（重新查询以确保最新）
		List<JjwxjcxxbEntity> existingJjgxxqbgls = detailService.getDetailsById(entityId);
